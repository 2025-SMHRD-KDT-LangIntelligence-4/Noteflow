...
public class FileStorageService {
	...
	public String storeFile(MultipartFile file, String userId) throws IOException {
		...
		GridFSUploadStream uploadStream = gridFSBucket.openUploadStream(storedFilename, options);
		uploadStream.write(file.getBytes());
		uploadStream.close();

		String gridId = uploadStream.getObjectId().toString();
		FileMetadata meta = FileMetadata.builder().originalName(filename).storedName(storedFilename)
				.fileSize(file.getSize()).mimeType(file.getContentType()).userId(userId).folderId(null)
				.uploadDate(LocalDateTime.now()).gridfsId(gridId).build();
		fileMetadataRepository.save(meta);
		return gridId;
	}

	// 선택한 폴더로 저장하려는 경우만 이걸 호출
	public String storeFile(MultipartFile file, String userId, String folderId) throws IOException {
		String filename = file.getOriginalFilename();
		String storedFilename = generateStoredFilename(filename);
		...
		String gridId = uploadStream.getObjectId().toString();
		FileMetadata meta = FileMetadata.builder().originalName(filename).storedName(storedFilename)
				.fileSize(file.getSize()).mimeType(file.getContentType()).userId(userId)
				.folderId((folderId == null || folderId.isBlank()) ? null : folderId) // ✅ 선택 폴더 반영
				.uploadDate(LocalDateTime.now()).gridfsId(gridId).build();
		fileMetadataRepository.save(meta);
		return gridId;
	}
	...
	public boolean deleteFile(String mongoDocId) {
		try {
			...
			// ✅ 메타데이터 동기 삭제
			fileMetadataRepository.findAll().stream().filter(m -> mongoDocId.equals(m.getGridfsId())).findFirst()
	...
	public List<FileInfo> listFiles() {
		...
		for (GridFSFile gf : files) {
			...
			list.add(new FileInfo(gf.getObjectId().toString(), gf.getFilename(),
					...
					gf.getUploadDate().toInstant().atZone(ZoneId.systemDefault()).toLocalDateTime(), uploaderIdValue // use
																														// uploaderIdValue
			));
		}
		return list;
	...
	public void writeFilesAsZip(List<String> ids, ZipOutputStream zos) throws IOException {
		for (String idStr : ids) {
			...
			String entryName = URLEncoder.encode(originalName, StandardCharsets.UTF_8).replaceAll("\\+", "%20");
