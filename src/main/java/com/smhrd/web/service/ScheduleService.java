...
public class ScheduleService {
	...
	/**
	 * ✅ 1-1. 선택 기간 동안 매일 동일한 시간대에 반복 일정을 등록합니다. 컨트롤러와 동일한 패턴을 위해 User 엔티티 대신
	 * userIdx를 매개변수로 받습니다.
	 * 
		...
		while (!currentDate.isAfter(periodEndDate)) {
			...
			if (requestDto.getAlarmTime() != null) {
				...
				long minutesDifference = java.time.Duration
						.between(requestDto.getStartTime(), requestDto.getAlarmTime()).toMinutes();
			...
			Schedule schedule = Schedule.builder().title(requestDto.getTitle()).description(requestDto.getDescription())
					.colorTag(requestDto.getColorTag()).isAllDay(requestDto.getIsAllDay()).startTime(scheduleStartTime) // 각
																														// 날짜의
																														// 시작
																														// 시간
					...
					// 추가 옵션 필드 매핑
					.emoji(requestDto.getEmoji()).alarmTime(newAlarmTime).alertType(requestDto.getAlertType())
					.customAlertValue(requestDto.getCustomAlertValue()).customAlertUnit(requestDto.getCustomAlertUnit())
					.location(requestDto.getLocation()).mapLat(requestDto.getMapLat()).mapLng(requestDto.getMapLng())
					.highlightType(requestDto.getHighlightType()).category(requestDto.getCategory())
					.attachmentPath(requestDto.getAttachmentPath()).attachmentList(requestDto.getAttachmentList())
					.isAllDay(requestDto.getIsAllDay())

					// 기본값 및 사용자 정보 설정
					.isDeleted(false).createdAt(LocalDateTime.now()).updatedAt(LocalDateTime.now()).user(user) // 위에서
																												// 조회한
																												// User
																												// 엔티티
																												// 사용
	...
	public List<Schedule> getSchedulesForPeriod(long userIdx, LocalDateTime start, LocalDateTime end) {
		return scheduleRepository
				.findByUser_UserIdxAndStartTimeLessThanEqualAndEndTimeGreaterThanEqualAndIsDeletedFalse(userIdx, end,
						start);
	}
	...
	public Optional<Schedule> getScheduleById(Long scheduleId) {
		return scheduleRepository.findById(scheduleId).filter(s -> s.getIsDeleted() == null || !s.getIsDeleted());
	}
